...
The one-lifetime-scout jump-BFS notebook run survived with both policies:
...
That local change did not generalize: the public score dropped from `1062.4`
...
Version 6 then reached `1228.8`, making active scout replacement the best
signal so far. Version 8 tested the small factory danger gate and dropped to
`1087.3`, so the next test should use V6 as the reference agent.
