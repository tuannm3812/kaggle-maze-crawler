...
4. `4_maze_crawler_miner_hybrid_agent.ipynb`
...
When notebook code changes, clear outputs in the same commit before pushing.
This keeps diffs reviewable and avoids committing large replay HTML. Kaggle is
the trusted execution record, so local notebook output should be regenerated by
rerunning the notebook on Kaggle.

If notebook outputs are intentionally committed, document why in the commit or
near the relevant notebook note. Docs-only changes do not require touching
notebook outputs.
